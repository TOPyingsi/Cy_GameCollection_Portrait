...
@ccclass('TZHZ_FuSshi')
export class TZHZ_FuSshi extends Component {
    ...
    onTouchEnd() {
        const PipingNode = instantiate(this.Piping);
        if (PipingNode.name == "帽子") {
            find("Canvas/工作台/服饰").addChild(PipingNode);
            PipingNode.setWorldPosition(find("Canvas/工作台/服饰").worldPosition.x, find("Canvas/工作台/服饰").worldPosition.y + 307.042, find("Canvas/工作台/服饰").worldPosition.z)
        }
        else {
            find("Canvas/工作台/服饰").addChild(PipingNode);
        }
        ...
        if (find("Canvas/工作台/服饰").children.length > 1) {
            find("Canvas/工作台/服饰").children[0].destroy();
        }
