import { _decorator, CCBoolean, Component, Label, Node, randomRangeInt, Sprite, Tween, tween, UIOpacity, v3, Vec3 } from 'cc';
...
@ccclass('THLCB_Chat')
export class THLCB_Chat extends Component {
    ...
    protected onEnable(): void {
        ...
        Tween.stopAllByTarget(this.node.children[1]);
        THLCB_AudioManager.Instance._PlaySound(45);
        this.node.setPosition(Vec3.ZERO);
        this.node.children[1].setScale(Vec3.ZERO);
        this.uiOpacity.opacity = 255;
        if (this.sprite) {
            if (THLCB_DataManager.chatSfs) this.sprite.spriteFrame = THLCB_DataManager.chatSfs[randomRangeInt(0, THLCB_DataManager.chatSfs.length)];
            else eventCenter.once("chatSfs", () => { this.sprite.spriteFrame = THLCB_DataManager.chatSfs[randomRangeInt(0, THLCB_DataManager.chatSfs.length)]; }, this);
