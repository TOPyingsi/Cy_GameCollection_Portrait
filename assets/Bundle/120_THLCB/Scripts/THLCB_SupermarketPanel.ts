...
@ccclass('THLCB_SupermarketPanel')
export class THLCB_SupermarketPanel extends THLCB_UIBase {

    private static instance: THLCB_SupermarketPanel;
    public static get Instance(): THLCB_SupermarketPanel {
    ...
    protected onLoad(): void {
        super.onLoad();
        THLCB_SupermarketPanel.instance = this;
    ...
    protected _InitData(): void {
        THLCB_AudioManager.Instance._PlaySPMKMusic();
        this.doorOpen = false;
    ...
    OpenDoors() {
        ...
        this.doorOpen = true;
        this.doors.play();
        THLCB_AudioManager.Instance._PlaySound(3);
    }
    ...
    ChooseItem(event: Event, num: number) {
        ...
        if (this.cost + price > coin) THLCB_MainPanel.Instance._OpenNeed();
    ...
    _ScanTouchMove(event: EventTouch) {
        ...
        if (this.scan.node.children[2].getComponent(UITransform).getBoundingBoxToWorld().contains(pos)) {
            THLCB_AudioManager.Instance._PlaySound(4);
    ...
    FinishDone() {
        THLCB_AudioManager.Instance._PlaySound(5);
        ...
        this.scheduleOnce(() => {
            ...
            THLCB_MainPanel.Instance.supermarketLivePanel.active = true;
        }, 2);
    }
