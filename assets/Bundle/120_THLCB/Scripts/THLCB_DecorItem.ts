...
import { THLCB_DecorPanel } from './THLCB_DecorPanel';
import Banner from 'db://assets/Scripts/Banner';
const { ccclass, property } = _decorator;

@ccclass('THLCB_DecorItem')
export class THLCB_DecorItem extends Component {
    ...
    _Init(_type = this.type, _num = this.num) {
        ...
        switch (_type) {
            ...
            case 1:
                ...
                this.player.animation = "daiji";
                preName = "Pet";
                this.nameLabel.string = THLCB_DataManager.allNames[1][_num];
                break;
            case 2:
        ...
        let data = THLCB_DataManager.Instance.getArrayData<number>(preName + "States")[_num];
        if (data == 1) {
            this.video.active = false;
            this.price.active = false;
            let data2 = THLCB_DataManager.Instance.getNumberData(preName);
            ...
            let cost = THLCB_DataManager.allDecorPrices[_type][_num];
    ...
    Choose() {
        ...
        switch (this.type) {
            ...
            case 1:
                preName = "Pet";
                THLCB_DecorPanel.Instance.buyPetData = this.pet.skeletonData;
                break;
            case 2:
                preName = "Bg";
                break;
        ...
        if (this.type > 1) THLCB_DecorPanel.Instance.buyIcon = this.icon.spriteFrame;
        let choose = THLCB_DataManager.Instance.getNumberData(preName);
        if (choose == this.num) return;
        let data = THLCB_DataManager.Instance.getArrayData<number>(preName + "States")[this.num];
        if (data == 1) THLCB_DecorPanel.Instance.ChooseDecor(this.type, this.num);
        else {
            let cost = THLCB_DataManager.allDecorPrices[this.type][this.num];
            if (cost == -1) THLCB_DecorPanel.Instance.VideoDecor(this.type, this.num);
            else THLCB_DecorPanel.Instance.BuyDecor(this.type, this.num);
        }
    }
