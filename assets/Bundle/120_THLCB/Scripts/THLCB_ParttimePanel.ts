...
@ccclass('THLCB_ParttimePanel')
export class THLCB_ParttimePanel extends THLCB_UIBase {
    ...
    protected _InitData(): void {
        THLCB_AudioManager.Instance._PlayPTMusic();
    ...
    StartGame() {
        ...
        this.scheduleOnce(() => {
            THLCB_AudioManager.Instance._PlaySound(23);
    ...
    _GuestReady() {
        ...
        this.guest.getComponent(Sprite).spriteFrame = THLCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_normal") return value; });
    ...
    _Win() {
        THLCB_AudioManager.Instance._PlaySound(24);
        this.isOrder = false;
        this.tf[0].active = true;
        this.guest.getComponent(Sprite).spriteFrame = THLCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_happy") return value; });
        this.GuestNum++;
    ...
    _Fail() {
        THLCB_AudioManager.Instance._PlaySound(25);
        this.isOrder = false;
        this.tf[1].active = true;
        this.guest.getComponent(Sprite).spriteFrame = THLCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_sad") return value; });
        this._GuestLeave();
    }
    ...
    VideoCoin() {
        ...
        Banner.Instance.ShowVideoAd(() => {
            ...
            THLCB_DataManager.coinSpawn = coinSpawn.getWorldPosition();
    ...
    EndCoin() {
        THLCB_AudioManager.Instance._PlaySound(1);
        THLCB_AudioManager.Instance._PlayMusic();
        THLCB_DataManager.coinSpawn = this.coinNode[1].getWorldPosition();
        let coin = THLCB_DataManager.Instance.getNumberData("Coin");
        coin += 800;
        THLCB_DataManager.Instance.setNumberData("Coin", coin);
        // let ani = this.finishPanel.getComponent(Animation);
