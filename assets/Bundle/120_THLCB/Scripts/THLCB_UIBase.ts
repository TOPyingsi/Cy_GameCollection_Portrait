...
@ccclass('THLCB_UIBase')
export class THLCB_UIBase extends Component {
    ...
    protected _UpdateLabel(label: Label, dataName: string, preStr = "", posStr = "") {
        let data = THLCB_DataManager.Instance.getNumberData(dataName);
        label.string = preStr + data + posStr;
    }

    protected _UpdateProgress(sprite: Sprite, dataName: string, limitName: string) {
        let data = THLCB_DataManager.Instance.getNumberData(dataName);
        let limit = limitName ? THLCB_DataManager.Instance.getNumberData(limitName) : 100;
