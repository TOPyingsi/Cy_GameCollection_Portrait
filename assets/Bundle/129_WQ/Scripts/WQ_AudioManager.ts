...
@ccclass('WQ_AudioManager')
export class WQ_AudioManager extends Component {
    private static instance: WQ_AudioManager;
    ...
    // 单例模式
    public static getInstance(): WQ_AudioManager {
        if (!WQ_AudioManager.instance) {
            // 查找场景中是否已存在GameManager节点
            const existingNode = director.getScene().getChildByName('GameManager');
            if (existingNode) {
                WQ_AudioManager.instance = existingNode.getComponent(WQ_AudioManager);
            } else {
                // 如果不存在，则创建新节点并添加组件
                const gameManagerNode = new Node('GameManager');
                director.getScene().addChild(gameManagerNode);
                WQ_AudioManager.instance = gameManagerNode.addComponent(WQ_AudioManager);
            }
        }
        return WQ_AudioManager.instance;
    }

    protected onLoad(): void {
        WQ_AudioManager.instance = this;
