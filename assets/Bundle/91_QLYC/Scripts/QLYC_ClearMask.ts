import { _decorator, director, Mask, Rect, rect, Sprite, UITransform, v3, Vec2, Vec3 } from 'cc';
import { QLYC_GameMgr } from './QLYC_GameMgr';
// import { QLED_GameMgr } from './QLED_GameMgr';
const { ccclass, property } = _decorator;

@ccclass('QLYC_ClearMask')
export default class QLYC_ClearMask extends Mask {
    ...
    start() {
        ...
        director.getScene().on("清理牙齿_开始清理", this.TouchMove, this);
        director.getScene().on("清理牙齿_清理结束", this.touchEnd, this);
    ...
    update(deltaTime: number) {
        if (this.startTimer) {
            ...
            if (this.timer >= 2) {
                console.log("强制结束");
                director.getScene().emit("清理牙齿_强制结束");

                director.getScene().off("清理牙齿_开始清理", this.TouchMove, this);
                director.getScene().off("清理牙齿_清理结束", this.touchEnd, this);
                ...
                if (this.maskIndex === 1) {
                    QLYC_GameMgr.instance.dirtyNode.active = false;
                    QLYC_GameMgr.instance.toothStart = true;
                }

                QLYC_GameMgr.instance.nextClear(this.maskIndex);
            }
