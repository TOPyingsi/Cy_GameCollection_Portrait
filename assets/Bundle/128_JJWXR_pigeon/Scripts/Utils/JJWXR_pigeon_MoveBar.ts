...
import { eventCenter } from './JJWXR_pigeon_EventCenter';
import { JJWXR_pigeon_Events } from './JJWXR_pigeon_Events';
...
export class JJWAR_MoveBar extends Component {
    ...
    start() {
        ...
        eventCenter.on(JJWXR_pigeon_Events.STOP_MOVE_BAR, this.onStop, this);  // 监听停止移动事件
    }

    onDestroy() {
        eventCenter.off(JJWXR_pigeon_Events.STOP_MOVE_BAR, this.onStop, this); // 取消监听停止移动事件
    }
    ...
    onStop() {
        this.isMoving = false;
        eventCenter.emit(JJWXR_pigeon_Events.GET_MORE_MONEY, this.money); // 发送获取更多金钱事件
    }
