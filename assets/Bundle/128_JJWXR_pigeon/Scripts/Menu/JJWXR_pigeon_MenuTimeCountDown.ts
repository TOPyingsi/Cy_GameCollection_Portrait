...
@ccclass('JJWXR_pigeon_MenuTimeCountDown')
export class JJWXR_pigeon_MenuTimeCountDown extends Component {
    ...
    start() {
        ...
        eventCenter.on(JJWXR_pigeon_Events.STOP_COUNTDOWN, this.stopCountdown, this); // 监听停止倒计时事件
        eventCenter.on(JJWXR_pigeon_Events.ENERGY_FULL, this.energyFull, this); // 监听开始倒计时事件
        eventCenter.on(JJWXR_pigeon_Events.ENERGY_NOT_FULL, this.energyNotFull, this); // 监听获取能量事件
    }

    onDestroy() {
        eventCenter.off(JJWXR_pigeon_Events.STOP_COUNTDOWN, this.stopCountdown, this); // 取消监听停止倒计时事件
        eventCenter.off(JJWXR_pigeon_Events.ENERGY_FULL, this.energyFull, this); // 取消监听开始倒计时事件
        eventCenter.off(JJWXR_pigeon_Events.ENERGY_NOT_FULL, this.energyNotFull, this); // 取消监听获取能量事件
    }
    // 每帧更新
    update() {
        ...
        if (remainingTime <= 0) {
            ...
            if (this.isEnergyFull) {
                ...
                eventCenter.emit(JJWXR_pigeon_Events.GET_ENERGY); // 发送获取能量事件
