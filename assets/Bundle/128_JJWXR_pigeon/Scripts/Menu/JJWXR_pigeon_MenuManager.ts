...
import { JJWXR_pigeon_EnergyManager } from './JJWXR_pigeon_EnergyManager';
import { LevelData } from './JJWXR_pigeon_Loading';
...
@ccclass('JJWXR_pigeon_MenuManager')
export class JJWXR_pigeon_MenuManager extends Component {
    ...
    // -------------------------------------------------------------------------------------------------------------------------------
    // 单例模式
    private static _instance: JJWXR_pigeon_MenuManager = null;
    public static get instance(): JJWXR_pigeon_MenuManager {
        return this._instance;
    }

    onLoad() {
        JJWXR_pigeon_MenuManager._instance = this;
    ...
    start() {
        ...
        const currentLevel = parseInt(localStorage.getItem('JJWXR_pigeon_currentLevel'));
        // if (isFirstPlay && currentLevel == 1) {
        //     console.log('第一次进入游戏MenuManager' + isFirstPlay + currentLevel);
        //     director.loadScene('JJWXR_pigeon_LoadingScene'); // 第一次进入游戏，加载加载场景
        // }
        // else {
        //     let isFirstPlay = JSON.parse(localStorage.getItem('JJWXR_pigeon_isFirstPlay'));
        //     const isPlay = false;
        //     localStorage.setItem('JJWXR_pigeon_isFirstPlay', JSON.stringify(isPlay));
        // }
        ...
        eventCenter.on(JJWXR_pigeon_Events.SHOW_ADD_ENERGY_UI, this.showAddEnergyUI, this);
        eventCenter.on(JJWXR_pigeon_Events.HIDE_ADD_ENERGY_UI, this.hideAddEnergyUI, this);
        this.WeaponShow();
    }

    onDestroy() {
        eventCenter.off(JJWXR_pigeon_Events.SHOW_ADD_ENERGY_UI, this.showAddEnergyUI, this);
        eventCenter.off(JJWXR_pigeon_Events.HIDE_ADD_ENERGY_UI, this.hideAddEnergyUI, this);
    ...
    playBackgroundMusic() {
        // 播放背景音乐
        let data = localStorage.getItem('JJWXR_pigeon_setting');
        let isPlayBGM = JSON.parse(data);
        JJWXR_pigeon_AudioManager.instance.play(this.backgroundMusic, isPlayBGM.sound);
    }
    ...
    onGameStart() {
        console.log('跳转到游戏界面');
        // // 判断是否可以开始游戏
        // if (JJWXR_pigeon_EnergyManager.instance.getEnergyNum() <= 0) {
        //     // 显示警告界面
        //     this.showWarningUI();
        //     return;
        // }
        // // 消耗能量
        // eventCenter.emit(JJWXR_pigeon_Events.USE_ENERGY);
        LevelData.GAMESCENE = "JJWXR_pigeon_GameScene00";
        // 跳转到加载游戏界面
        director.loadScene('JJWXR_pigeon_LoadingScene');
    }
    ...
    WeaponShow(event?: Event) {
        ...
        if (num == 0) {
            ...
            if (!localStorage.getItem('JJWXR_pigeon_gunIndex')) localStorage.setItem('JJWXR_pigeon_gunIndex', "1");
        }
        ...
        let index = parseInt(localStorage.getItem('JJWXR_pigeon_gunIndex'));
    ...
    BuyGun() {
        ...
        eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, price);
    ...
    UseGun() {
        localStorage.setItem('JJWXR_pigeon_gunIndex', this.weaponNum.toString());
        let index = parseInt(localStorage.getItem('JJWXR_pigeon_gunIndex'));
        this.weaponUI.children[3].active = !this.gunData[this.weaponNum - 1].isBuy;
        this.weaponUI.children[3].children[0].getComponent(Label).string = this.gunData[this.weaponNum - 1].price.toString();
        this.weaponUI.children[4].active = this.gunData[this.weaponNum - 1].isBuy && index != this.weaponNum;
