...
import { JJWXR_pigeon_MoneyManager } from './JJWXR_pigeon_MoneyManager';
const { ccclass, property } = _decorator;

@ccclass('JJWXR_pigeon_Armory')
export class JJWXR_pigeon_Armory extends Component {
    ...
    start() {
        ...
        this.initIndexData();
        this.updateBtnState();
        eventCenter.on(JJWXR_pigeon_Events.UPDATE_MONEY, this.updateMoney, this);
        eventCenter.on(JJWXR_pigeon_Events.UPDATE_BUTTON_STATE, this.updateBtnState, this);
    }
    protected onDestroy(): void {
        eventCenter.off(JJWXR_pigeon_Events.UPDATE_MONEY, this.updateMoney, this);
        eventCenter.off(JJWXR_pigeon_Events.UPDATE_BUTTON_STATE, this.updateBtnState, this);
    }
    ...
    onUseClick(event: Event) {
        ...
        eventCenter.emit(JJWXR_pigeon_Events.GUN_CHANGE); // 发送事件，更新当前使用枪
    }
    ...
    onCanBuyClick(event: Event) {
        ...
        switch (btnIndex) {
            case 1:
                const gun1 = JSON.parse(localStorage.getItem('gun1'));
                if (gun1.price > JJWXR_pigeon_MoneyManager.Money) return
                gun1.isBuy = true;
                localStorage.setItem('gun1', JSON.stringify(gun1));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun1.price);   // 扣钱
            ...
            case 2:
                const gun2 = JSON.parse(localStorage.getItem('gun2'));
                if (gun2.price > JJWXR_pigeon_MoneyManager.Money) return
                gun2.isBuy = true;
                localStorage.setItem('gun2', JSON.stringify(gun2));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun2.price);   // 扣钱
                button.getChildByName('btnCanNotBuy').active = false;   // 隐藏不可购买按钮
                button.getChildByName('btnCanBuy').active = false;   // 隐藏购买按钮
                button.getChildByName('btnCanUse').active = true;   // 显示使用按钮
                break;
            case 3:
                const gun3 = JSON.parse(localStorage.getItem('gun3'));
                if (gun3.price > JJWXR_pigeon_MoneyManager.Money) return
                gun3.isBuy = true;
                localStorage.setItem('gun3', JSON.stringify(gun3));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun3.price);   // 扣钱
                button.getChildByName('btnCanNotBuy').active = false;   // 隐藏不可购买按钮
                button.getChildByName('btnCanBuy').active = false;   // 隐藏购买按钮
                button.getChildByName('btnCanUse').active = true;   // 显示使用按钮
                break;
            case 4:
                const gun4 = JSON.parse(localStorage.getItem('gun4'));
                if (gun4.price > JJWXR_pigeon_MoneyManager.Money) return
                gun4.isBuy = true;
                localStorage.setItem('gun4', JSON.stringify(gun4));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun4.price);   // 扣钱
                button.getChildByName('btnCanNotBuy').active = false;   // 隐藏不可购买按钮
                button.getChildByName('btnCanBuy').active = false;   // 隐藏购买按钮
                button.getChildByName('btnCanUse').active = true;   // 显示使用按钮
                break;
            case 5:
                const gun5 = JSON.parse(localStorage.getItem('gun5'));
                if (gun5.price > JJWXR_pigeon_MoneyManager.Money) return
                gun5.isBuy = true;
                localStorage.setItem('gun5', JSON.stringify(gun5));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun5.price);   // 扣钱
                button.getChildByName('btnCanNotBuy').active = false;   // 隐藏不可购买按钮
                button.getChildByName('btnCanBuy').active = false;   // 隐藏购买按钮
                button.getChildByName('btnCanUse').active = true;   // 显示使用按钮
                break;
            case 6:
                const gun6 = JSON.parse(localStorage.getItem('gun6'));
                if (gun6.price > JJWXR_pigeon_MoneyManager.Money) return
                gun6.isBuy = true;
                localStorage.setItem('gun6', JSON.stringify(gun6));
                eventCenter.emit(JJWXR_pigeon_Events.SUB_MONEY, gun6.price);   // 扣钱
                button.getChildByName('btnCanNotBuy').active = false;   // 隐藏不可购买按钮
                button.getChildByName('btnCanBuy').active = false;   // 隐藏购买按钮
                button.getChildByName('btnCanUse').active = true;   // 显示使用按钮
                break;
