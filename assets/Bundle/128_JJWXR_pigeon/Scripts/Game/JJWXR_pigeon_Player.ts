import { _decorator, Component, Node, Vec2, EventTouch, Vec3, Input, Camera, Prefab, instantiate, tween, AudioClip, randomRange, director, geometry, PhysicsRayResult, PhysicsSystem } from 'cc';
...
import { JJWXR_pigeon_BattleUI } from './UI/JJWXR_pigeon_BattleUI';
import { PoolManager } from 'db://assets/Scripts/Framework/Managers/PoolManager';
import { JJWXR_pigeon_Bullet } from './JJWXR_pigeon_Bullet';
import { JJWXR_pigeon_Enemy } from './JJWXR_pigeon_Enemy';
import { JJWXR_pigeon_SucceedUI } from './UI/JJWXR_pigeon_SucceedUI';
const { ccclass, property } = _decorator;

@ccclass('JJWXR_pigeon_Player')
export class JJWXR_pigeon_Player extends Component {
    // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    @property(Node)
    ...
    // ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    // 单例模式
    private static _instance: JJWXR_pigeon_Player = null;
    public static get instance(): JJWXR_pigeon_Player {
        return this._instance;
    }

    onLoad() {
        console.log('初始化');
        JJWXR_pigeon_Player._instance = this; // 单例模式
        this.onTouchEventStart();
        eventCenter.on(JJWXR_pigeon_Events.ON_FIRE, this.onFire, this); // 监听发射子弹事件
    }
    ...
    start() {
        ...
        eventCenter.on(JJWXR_pigeon_Events.ON_TOUCH_EVENT_START, this.onTouchEventStart, this); // 监听触摸事件
        eventCenter.on(JJWXR_pigeon_Events.ON_TOUCH_EVENT_END, this.onTouchEventEnd, this); // 监听触摸事件
        eventCenter.on(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV, this.onChangeCameraFov, this); // 监听改变摄像机视野大小事件
        eventCenter.on(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_BACK, this.onChangeCameraFovBack, this); // 监听改变摄像机视野大小事件
        eventCenter.on(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_SPEED, this.onChangeCameraFovSpeed, this); // 监听改变摄像机视野大小事件
        eventCenter.on(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_SPEED_BACK, this.onChangeCameraFovSpeedBack, this); // 监听改变摄像机视野大小事件

        eventCenter.on(JJWXR_pigeon_Events.ON_SPAWN_RING, this.onSpawnRing, this); // 生成环
        eventCenter.on(JJWXR_pigeon_Events.ON_REMOVE_RING, this.onRemoveRing, this); // 移除环

        eventCenter.emit(JJWXR_pigeon_Events.LOOKAT_TARGET_POSITION, this.lookAtTargetPosition, this); // 发射子弹事件初始化摄像机视野大小

        eventCenter.on(JJWXR_pigeon_Events.MOVE_TO_ORIGIN_POSITION, this.moveToOriginPosition, this); // 移动到初始位置
    }

    onDestroy() {
        eventCenter.off(JJWXR_pigeon_Events.ON_TOUCH_EVENT_START, this.onTouchEventStart, this); // 取消监听触摸事件
        eventCenter.off(JJWXR_pigeon_Events.ON_TOUCH_EVENT_END, this.onTouchEventEnd, this); // 取消监听触摸事件
        eventCenter.off(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV, this.onChangeCameraFov, this); // 取消监听改变摄像机视野大小事件
        eventCenter.off(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_BACK, this.onChangeCameraFovBack, this); // 监听改变摄像机视野大小事件
        eventCenter.off(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_SPEED, this.onChangeCameraFovSpeed, this); // 取消监听改变摄像机视野大小事件
        eventCenter.off(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_SPEED_BACK, this.onChangeCameraFovSpeedBack, this); // 监听改变摄像机视野大小事件
        eventCenter.off(JJWXR_pigeon_Events.ON_FIRE, this.onFire, this); // 取消监听发射子弹事件

        eventCenter.off(JJWXR_pigeon_Events.ON_SPAWN_RING, this.onSpawnRing, this); // 生成环
        eventCenter.off(JJWXR_pigeon_Events.ON_REMOVE_RING, this.onRemoveRing, this); // 移除环

        eventCenter.off(JJWXR_pigeon_Events.LOOKAT_TARGET_POSITION, this.lookAtTargetPosition, this); // 取消监听发射子弹事件

        eventCenter.off(JJWXR_pigeon_Events.MOVE_TO_ORIGIN_POSITION, this.moveToOriginPosition, this); // 取消监听移动到初始位置
    }
    ...
    private onTouchStart(event: EventTouch) {
        ...
        eventCenter.emit(JJWXR_pigeon_Events.ONTOUCHSTART_BATTLEUI, touchLocation); // 发送触摸开始事件
    }
    ...
    private onTouchMove(event: EventTouch) {
        ...
        // eventCenter.emit(JJWXR_pigeon_Events.ONTOUCHMOVE_BATTLEUI, this.cameraNode.eulerAngles); // 发送触摸移动事件
    }
    ...
    private onTouchEnd(event: EventTouch) {
        ...
        if (this.startPos.x === touchLocation.x && this.startPos.y === touchLocation.y) {
            console.log('点击');
            // JJWXR_pigeon_BattleUI.instance.onTouchEndBattleUI();
            eventCenter.emit(JJWXR_pigeon_Events.ONTOUCHEND_BATTLEUI); // 发送触摸结束事件
        } else {
            console.log('滑动');
        }
    ...
    onFire() {
        this.fireRaycast();
        // 添加开火音效
        JJWXR_pigeon_AudioManager.instance.playOneShot(this.fireSound);
    }
    ...
    fireRaycast() {
        ...
        const ray = new geometry.Ray(
            this.cameraNode.getWorldPosition().x, this.cameraNode.getWorldPosition().y, this.cameraNode.getWorldPosition().z, // 起点
            this.cameraNode.forward.clone().x, this.cameraNode.forward.clone().y, this.cameraNode.forward.clone().z // 方向
        );
        ...
        if (PhysicsSystem.instance.raycast(ray)) {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (hit.collider.node.parent.getComponent(JJWXR_pigeon_Enemy).isDie) continue;
                console.log("子弹击中了物体, 分组为: " + hitNode.getGroup());
                if ((hitNode.getGroup() === 4 || hitNode.getGroup() === 8) && JJWXR_pigeon_Bullet.isBulletTime) {
                    eventCenter.emit(JJWXR_pigeon_Events.HIDE_GAME_UI);
                    ...
                    bullet.getComponent(JJWXR_pigeon_Bullet).Init(this.hitPos);
                    break;
                }
                else {
    ...
    // 检查命中的物体
    checkHit(hit: PhysicsRayResult, index: number) {
        if (index === 4 || index === 8) {
            // 播放精确命中特效
        ...
        if (index === 4) {
            ...
            JJWXR_pigeon_SucceedUI.headShot++;
            JJWXR_pigeon_SucceedUI.hitShot++;

            // 播放精确命中动画
            eventCenter.emit(JJWXR_pigeon_Events.SHOWPRECISEHIT);
            console.log("精确命中");
            // 敌人死亡
            // eventCenter.emit(JJWXR_pigeon_Events.ENEMY_DESTROY, hit.collider.node.parent);
            hit.collider.node.parent.getComponent(JJWXR_pigeon_Enemy).destroyEnemy();
        }
        else if (index === 8) {
            // 播放命中动画
            // 敌人死亡
