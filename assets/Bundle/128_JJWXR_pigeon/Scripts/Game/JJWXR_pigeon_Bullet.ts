...
import { eventCenter } from '../Utils/JJWXR_pigeon_EventCenter';//自定义事件中心，用于模块间通信。
import { JJWXR_pigeon_Events } from '../Utils/JJWXR_pigeon_Events';//自定义事件枚举
import { JJWXR_pigeon_Player } from './JJWXR_pigeon_Player';
import { JJWXR_pigeon_SucceedUI } from './UI/JJWXR_pigeon_SucceedUI';//游戏结算 UI 脚本
import { JJWXR_pigeon_Enemy } from './JJWXR_pigeon_Enemy';
const { ccclass, property } = _decorator;

@ccclass('JJWXR_pigeon_Bullet')
export class JJWXR_pigeon_Bullet extends Component {
    ...
    Init(hit: PhysicsRayResult) {
        ...
        // // 射线检测
        // this.fireRaycast();
        // if (!JJWXR_pigeon_Bullet.isBulletTime) {
        //     // 自动销毁子弹
        //     this.scheduleOnce(() => {
        //         this.destroyBullet();
        //     }, this.lifeTime);
    ...
    //     // event.otherCollider.node.getComponent(JJWXR_pigeon_Player); // 获取碰撞的节点
    //     this.spawnHieEffect(this.hitPos);
    //     this.camera.node.setParent(director.getScene());
    //     this.destroyBullet(); // 销毁子弹
    //     eventCenter.emit(JJWXR_pigeon_Events.ENEMY_DESTROY, this.hitPos.collider.node.parent);
    // }
    ...
    moveBulletTime() {
        ...
        eventCenter.emit(JJWXR_pigeon_Events.HIDE_RETICLE_UI);
        // 使用刚体组件移动子弹
        this.rigidBody = this.node.getComponent(RigidBody);
    ...
    followTarget() {
        ...
        JJWXR_pigeon_Player.instance.camera.enabled = false; // 禁用玩家摄像机
        // 获取摄像机节点
                ...
                if ((hitNode.getGroup() === 4 || hitNode.getGroup() === 8) && JJWXR_pigeon_Bullet.isBulletTime) {
                    ...
                    tween(this.node)
                        ...
                        .call(() => {
                            ...
                            this.destroyBullet(); // 销毁子弹
                            // eventCenter.emit(JJWXR_pigeon_Events.ENEMY_DESTROY, this.hitPos.collider.node.parent);
                            this.hitPos.collider.node.parent.getComponent(JJWXR_pigeon_Enemy).destroyEnemy();
                        })
                        .start();
                    // 开启摄像机跟随
                    console.log("开启摄像机跟随");
    ...
    checkHit(hit: PhysicsRayResult, index: number) {
        ...
        // 销毁子弹
        if (!JJWXR_pigeon_Bullet.isBulletTime) this.destroyBullet();
    ...
    Run() {
        eventCenter.emit(JJWXR_pigeon_Events.HIDE_GAME_UI);
        tween(this.node)
            .to(1.5, { worldPosition: this.hitPos.hitPoint })
            .call(() => {
                this.spawnHieEffect(this.hitPos.hitPoint);
                this.camera.node.setParent(director.getScene(), true);
                this.destroyBullet(); // 销毁子弹
