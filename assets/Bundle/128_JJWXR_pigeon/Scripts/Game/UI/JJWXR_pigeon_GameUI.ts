...
import { JJWXR_pigeon_UIManager } from './JJWXR_pigeon_UIManager';
...
@ccclass('JJWXR_pigeon_GameUI')
export class JJWXR_pigeon_GameUI extends Component {
    ...
    start() {
        ...
        eventCenter.on(JJWXR_pigeon_Events.HIDE_ENEMY_UI, this.hideEnemyUI, this); // 开始订阅敌人UI隐藏事件
        eventCenter.on(JJWXR_pigeon_Events.HIDE_RETICLE_UI, this.hideReticleUI, this); //  开始订阅准星UI显示事件
    }

    protected onDestroy(): void {
        eventCenter.off(JJWXR_pigeon_Events.HIDE_ENEMY_UI, this.hideEnemyUI, this);    //  取消订阅敌人UI隐藏事件
        eventCenter.off(JJWXR_pigeon_Events.HIDE_RETICLE_UI, this.hideReticleUI, this); // 取消订阅准星UI显示事件
    }

    init() {
        let curLevel = parseInt(localStorage.getItem('JJWXR_pigeon_currentLevel')); // 获取当前关卡
        this.level = curLevel;
    ...
    onAim() {
        console.log('开镜');
        JJWXR_pigeon_UIManager.instance.showBattleUI();
        JJWXR_pigeon_UIManager.instance.hideGameUI();
        eventCenter.emit(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV);
        eventCenter.emit(JJWXR_pigeon_Events.CHANGE_CAMERA_FOV_SPEED);
    }
    ...
    onCheat01() {
        ...
        Banner.Instance.ShowVideoAd(() => {
            console.log('作弊按钮01');
            x.showEnemyPic();
            eventCenter.emit(JJWXR_pigeon_Events.ENEMY_PICTURE);
        })
    }
    ...
    onCheat02() {
        Banner.Instance.ShowVideoAd(() => {
            console.log('作弊按钮02');
            eventCenter.emit(JJWXR_pigeon_Events.ENEMY_WORLDPOSITION);
            eventCenter.emit(JJWXR_pigeon_Events.ON_SPAWN_RING);
        });
    }
    ...
    // 显示设置UI
    showSettingUI() {
        eventCenter.emit(JJWXR_pigeon_Events.GAME_STOP);
