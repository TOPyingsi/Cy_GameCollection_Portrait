...
@ccclass('JJWXR_pigeon_FailedUI')
export class JJWXR_pigeon_FailedUI extends Component {
    @property(Button)
    ...
    onLoad() {
        ...
        eventCenter.on(JJWXR_pigeon_Events.SHOW_WARNING_UI, this.showWarningUI, this);
    }
    onDestroy() {
        eventCenter.off(JJWXR_pigeon_Events.SHOW_WARNING_UI, this.showWarningUI, this);
    }
    ...
    onComeBack() {
        eventCenter.emit(JJWXR_pigeon_Events.LOAD_MENU_SCENE);
    }

    onAddTime() {
        let x = this;
        Banner.Instance.ShowVideoAd(() => {
            eventCenter.emit(JJWXR_pigeon_Events.GAME_START, 180);
            x.node.active = false;
        });
    }
    ...
    onRestart() {
        ...
        eventCenter.emit(JJWXR_pigeon_Events.RESTART_GAME);
    }
