...
import { JJWXR_pigeon_RamainManager } from './JJWXR_pigeon_RamainManager';
const { ccclass, property } = _decorator;

@ccclass('JJWXR_pigeon_SucceedUI')
export class JJWXR_pigeon_SucceedUI extends Component {
    ...
    @property(JJWXR_pigeon_RamainManager)
    enemyRemainManager: JJWXR_pigeon_RamainManager = null;
    ...
    start() {
        ...
        let currentLevel = (localStorage.getItem('JJWXR_pigeon_currentLevel'));
        console.log('level:', currentLevel); // 获取当前关卡

        let data = localStorage.getItem('JJWXR_pigeon_progress');
        let progress = { total: 3, current: 0, gunIndex: 1 };
        if (data != null && data != "") progress = JSON.parse(data);
        localStorage.setItem('JJWXR_pigeon_progress', JSON.stringify(progress)); // 初始化进度
    ...
    onComeBack() {
        this.onceTouch = true;
        if (this.onceTouch) {
            eventCenter.emit(JJWXR_pigeon_Events.GET_SUCCEED_MONEY);   // 获取奖励  
        }
    ...
    onMoreReward() {
        ...
        Banner.Instance.ShowVideoAd(() => {
            if (!x.onceTouch) {
                eventCenter.emit(JJWXR_pigeon_Events.GET_MORE_REWARD_SCENE, 300);   // 更多奖励
                // eventCenter.emit(JJWXR_pigeon_Events.STOP_MOVE_BAR);   // 获取奖励
    ...
    // 关卡切换
    onNextLevel() {
        let currentLevel = parseInt(localStorage.getItem('JJWXR_pigeon_currentLevel'));
        currentLevel++;
        localStorage.setItem('JJWXR_pigeon_currentLevel', (currentLevel).toString());
        ...
        // JJWXR_pigeon_SucceedUI.headShot = 0;
        // JJWXR_pigeon_SucceedUI.hitShot = 0;
        // // 数据归零
        ...
        // if (currentLevel < 6) {
        // let curlevel = JSON.parse(localStorage.getItem('JJWXR_pigeon_currentLevel'));
    ...
    addProgress() {
        return;
        const progress = JSON.parse(localStorage.getItem('JJWXR_pigeon_progress'));
        ...
        localStorage.setItem('JJWXR_pigeon_progress', JSON.stringify(progress));
        ...
        if (progress.current >= progress.total && progress.gunIndex == 1) {
            ...
            eventCenter.emit(JJWXR_pigeon_Events.UPDATE_BUTTON_STATE);   // 更多奖励
