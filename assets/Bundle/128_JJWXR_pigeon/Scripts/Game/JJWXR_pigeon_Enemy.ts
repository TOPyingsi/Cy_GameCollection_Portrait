...
import { JJWXR_pigeon_RamainManager } from './UI/JJWXR_pigeon_RamainManager';
const { ccclass, property } = _decorator;

@ccclass('JJWXR_pigeon_Enemy')
export class JJWXR_pigeon_Enemy extends Component {
    @property(AudioClip)
    ...
    start() {
        ...
        // eventCenter.on(JJWXR_pigeon_Events.ENEMY_DESTROY, this.destroyEnemy, this);
        // eventCenter.on(JJWXR_pigeon_Events.ENEMY_CHANGE_MATERIAL, this.changeMaterial, this);
        // eventCenter.on(JJWXR_pigeon_Events.ENEMY_CHANGE_ROLE, this.changeRole, this);
    }

    onDestroy() {
        // eventCenter.off(JJWXR_pigeon_Events.ENEMY_DESTROY, this.destroyEnemy, this);
        // eventCenter.off(JJWXR_pigeon_Events.ENEMY_CHANGE_MATERIAL, this.changeMaterial, this);
        // eventCenter.off(JJWXR_pigeon_Events.ENEMY_CHANGE_ROLE, this.changeRole, this);
    }
    ...
    public destroyEnemy(hitNode?: Node) {
        ...
        if (!this.isDie) {
            // 数量减少
            // eventCenter.emit(JJWXR_pigeon_Events.ENEMY_REDUSE);
            JJWXR_pigeon_RamainManager.instance.remainCountReduse();
            this.isDie = true;
        }

        let isFirstPlay = localStorage.getItem('JJWXR_pigeon_isFirstPlay');
        if (isFirstPlay === 'true') {
            eventCenter.emit(JJWXR_pigeon_Events.SHOW_ENCOURAGE_UI); // 显示鼓励UI
            localStorage.setItem('JJWXR_pigeon_isFirstPlay', "false"); // 存储为字符串
        }

        eventCenter.emit(JJWXR_pigeon_Events.HIDE_ENEMY_UI, this.node);
        eventCenter.emit(JJWXR_pigeon_Events.ON_REMOVE_RING, this.node);
        const child = this.node.parent.children;
        // 播放音效
        JJWXR_pigeon_AudioManager.instance.playOneShot(this.beHitSource);
        // 播放动画
        this.playAnimation();
        // // 改变子节点颜色
        // this.changeColor();
        this.oldEnemy.children[0].getComponent(SkinnedMeshRenderer).material = this.enemyDefaultMaterial;
        this.scheduleOnce(() => {
            this.node.destroy();
        }, 2);
        // }
