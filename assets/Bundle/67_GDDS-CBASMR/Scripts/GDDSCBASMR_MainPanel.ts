...
import { GDDSCBASMR_Coin } from './GDDSCBASMR_Coin';
import { GDDSCBASMR_AudioManager } from './GDDSCBASMR_AudioManager';
import { ProjectEvent, ProjectEventManager } from 'db://assets/Scripts/Framework/Managers/ProjectEventManager';
const { ccclass, property } = _decorator;

@ccclass('GDDSCBASMR_MainPanel')
export class GDDSCBASMR_MainPanel extends GDDSCBASMR_UIBase {

    private static instance: GDDSCBASMR_MainPanel;
    public static get Instance(): GDDSCBASMR_MainPanel {
    ...
    protected onLoad(): void {
        GDDSCBASMR_MainPanel.instance = this;
    ...
    protected _InitUpdateEvent() {
        eventCenter.on("SetData:GDDSCBASMR_Level", this._UpdateLevel, this);
        eventCenter.on("SetData:GDDSCBASMR_Coin", this._UpdateCoin, this);
        eventCenter.on("SetData:GDDSCBASMR_Exp", this._UpdateExp, this);
        eventCenter.on("SetData:GDDSCBASMR_Fans", this._UpdateFans, this);
        eventCenter.on("SetData:GDDSCBASMR_Skin", this._UpdateSkin, this);
        eventCenter.on("SetData:GDDSCBASMR_Pet", this._UpdatePet, this);
        eventCenter.on("SetData:GDDSCBASMR_Bg", this._UpdateBg, this);
        eventCenter.on("SetData:GDDSCBASMR_Table", this._UpdateTable, this);
        eventCenter.on("SetData:GDDSCBASMR_Decor", this._UpdateDecor, this);
    }
    ...
    _UpdateCoin() {
        GDDSCBASMR_DataManager.deltaCoin = GDDSCBASMR_DataManager.Instance.getNumberData("Coin") - parseInt(this.coinLabel.string);
        if (parseInt(this.coinLabel.string) == -1) this._UpdateLabel(this.coinLabel, "Coin");
        else {
            let num = Math.floor(GDDSCBASMR_DataManager.deltaCoin / 50);
            if (GDDSCBASMR_DataManager.deltaCoin > 0) {
                this.coinAddLabel.string = "+" + GDDSCBASMR_DataManager.deltaCoin;
                this.coinAddLabel.node.setWorldPosition(GDDSCBASMR_DataManager.coinSpawn);
            ...
            if (Math.abs(GDDSCBASMR_DataManager.deltaCoin) > 1000) {
                if (GDDSCBASMR_DataManager.deltaCoin > 0) {
                    this.schedule(() => {
                        let coin: Node = PoolManager.GetNodeByPrefab(this.coinPrefab, GDDSCBASMR_MainPanel.Instance.coinPanel);
                        coin.setWorldPosition(GDDSCBASMR_DataManager.coinSpawn);
                        if (GDDSCBASMR_DataManager.deltaCoin > num) {
                            GDDSCBASMR_DataManager.deltaCoin -= num;
                            coin.getComponent(GDDSCBASMR_Coin).price = num;
                        }
                        else {
                            coin.getComponent(GDDSCBASMR_Coin).price = GDDSCBASMR_DataManager.deltaCoin;
                            GDDSCBASMR_DataManager.deltaCoin = 0;
                        }
                    }, 0.02, 49);
                }
                else {
                    ...
                    this.schedule(() => {
                        let price = 0;
                        if (GDDSCBASMR_DataManager.deltaCoin < -num2) {
                            GDDSCBASMR_DataManager.deltaCoin += num2;
                            price = -num2;
                        }
                        else {
                            price = GDDSCBASMR_DataManager.deltaCoin;
                            GDDSCBASMR_DataManager.deltaCoin = 0;
                        }
                ...
                if (GDDSCBASMR_DataManager.deltaCoin > 0) {
                    this.schedule(() => {
                        if (GDDSCBASMR_DataManager.deltaCoin <= 0) return;
                        let coin: Node = PoolManager.GetNodeByPrefab(this.coinPrefab, GDDSCBASMR_MainPanel.Instance.coinPanel);
                        coin.setWorldPosition(GDDSCBASMR_DataManager.coinSpawn);
                        if (GDDSCBASMR_DataManager.deltaCoin >= 50) {
                            GDDSCBASMR_DataManager.deltaCoin -= 50;
                            coin.getComponent(GDDSCBASMR_Coin).price = 50;
                        }
                        else if (GDDSCBASMR_DataManager.deltaCoin > 0) {
                            coin.getComponent(GDDSCBASMR_Coin).price = GDDSCBASMR_DataManager.deltaCoin;
                            GDDSCBASMR_DataManager.deltaCoin = 0;
                        }
                    }, 0.02, num);
    ...
    _UpdateExp() {
        ...
        if (this.expSprite.fillRange >= 1) {
            ...
            level++;
            GDDSCBASMR_DataManager.Instance.setNumberData("Level", level);
            let exp = GDDSCBASMR_DataManager.Instance.getNumberData("Exp");
            let limit = GDDSCBASMR_DataManager.Instance.getNumberData("ExpLimit");
            exp -= limit;
            GDDSCBASMR_DataManager.Instance.setNumberData("Exp", exp);
            limit += 100;
            GDDSCBASMR_DataManager.Instance.setNumberData("ExpLimit", limit);
            this._OpenUpgrade();
        }
    }
    ...
    _UpdatePet() {
        let num = GDDSCBASMR_DataManager.Instance.getNumberData("Pet");
        if (num == -1) return;
    ...
    _UpdateBg() {
        let num = GDDSCBASMR_DataManager.Instance.getNumberData("Bg");
        this.bg.spriteFrame = this.decorBgSfs[num];
    }

    _UpdateTable() {
        let num = GDDSCBASMR_DataManager.Instance.getNumberData("Table");
        this.table.spriteFrame = this.decorTableSfs[num];
    }

    _UpdateDecor() {
        let num = GDDSCBASMR_DataManager.Instance.getNumberData("Decor");
    ...
    _UpdateFans() {
        ...
        if (num >= GDDSCBASMR_DataManager.fanLevelNums[level + 1]) {
            level++;
            GDDSCBASMR_DataManager.Instance.setNumberData("FanLevel", level);
    ...
    ClickPet() {
        ...
        let data = GDDSCBASMR_DataManager.Instance.getNumberData("Pet");
        GDDSCBASMR_AudioManager.Instance._PlaySound(26 + data);
        this.isPet = true;
    ...
    _Fade(arg: Node | Function, openPanel?: Node) {
        GDDSCBASMR_AudioManager.Instance._PlaySound(52);
