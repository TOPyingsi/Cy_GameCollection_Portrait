...
@ccclass('GDDSCBASMR_GachaPanel')
export class GDDSCBASMR_GachaPanel extends GDDSCBASMR_UIBase {
    ...
    protected _InitData(): void {
        this.isGacha = false;
        let data = GDDSCBASMR_DataManager.Instance.getArrayData<number>("SkinStates");
        this.fReward.children[0].active = data[28] == 1;
        this.fReward.children[1].active = data[28] == 0;
        let lastGacha = GDDSCBASMR_DataManager.Instance.getArrayData<number>("LastGacha");
        let gacha = GDDSCBASMR_DataManager.Instance.getNumberData("Gacha");
        let date = new Date;
        if (lastGacha.length == 0 || !((lastGacha.length > 0 && lastGacha[0] == date.getFullYear() && lastGacha[1] == date.getMonth() && lastGacha[2] == date.getDate()))) gacha = 0, GDDSCBASMR_DataManager.Instance.setNumberData("Gacha", gacha);
    ...
    _Gacha() {
        GDDSCBASMR_AudioManager.Instance._PlaySound(21);
        ...
        GDDSCBASMR_DataManager.Instance.setArrayData("LastGacha", lastGacha);
        let gacha = GDDSCBASMR_DataManager.Instance.getNumberData("Gacha");
        gacha++;
        GDDSCBASMR_DataManager.Instance.setNumberData("Gacha", gacha);
        let ani = this.gachaBtn.getComponent(Animation);
        if (gacha < 4) ani.play();
        else ani.stop();
        this.gachaBtn.getComponent(Button).interactable = gacha < 4;
    ...
    CloseReward(event: Event) {
        ...
        if (this.reward == 2000 && data[28] == 0) {
            data[28] = 1;
            GDDSCBASMR_DataManager.Instance.setArrayData("SkinStates", data);
        }
        else {
            GDDSCBASMR_DataManager.coinSpawn = event.target.getWorldPosition();
            let money = GDDSCBASMR_DataManager.Instance.getNumberData("Coin");
            money += this.reward;
            GDDSCBASMR_DataManager.Instance.setNumberData("Coin", money);
        }
        let ani = this.rewardPanel.getComponent(Animation);
