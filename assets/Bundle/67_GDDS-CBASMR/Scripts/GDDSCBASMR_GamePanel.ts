...
@ccclass('GDDSCBASMR_GamePanel')
export class GDDSCBASMR_GamePanel extends GDDSCBASMR_UIBase {

    private static instance: GDDSCBASMR_GamePanel;
    public static get Instance(): GDDSCBASMR_GamePanel {
    ...
    protected onLoad(): void {
        super.onLoad();
        GDDSCBASMR_GamePanel.instance = this;
        this._InitTouch();
    }
    ...
    TouchStart(event: EventTouch) {
        ...
        if (this.step == 2) {
            GDDSCBASMR_AudioManager.Instance._PLayLoopSound(47);
            ...
            GDDSCBASMR_AudioManager.Instance._PLayLoopSound(48);
            this.tutorials.children[1].active = false;
            this.top2.setWorldPosition(v3(pos.x, this.top2.getWorldPosition().y));
            let particles = this.top2.children[GDDSCBASMR_DataManager.curMubkang[3] - 1].getComponentsInChildren(ParticleSystem2D);
            for (let i = 0; i < particles.length; i++) {
                const element = particles[i];
                element.resetSystem();
            }
    ...
    TouchEnd(event: EventTouch) {
        if (!this.timeProcess.node.parent.active) return;
        GDDSCBASMR_AudioManager.Instance._StopLoopSound();
        this.isTouch = false;
        if (this.step == 2) {
    ...
    protected _InitData(): void {
        GDDSCBASMR_AudioManager.Instance._PlayCookMusic();
        ...
        GDDSCBASMR_DataManager.curMubkang = [-1, -1, -1, -1];
    ...
    _InitRequest() {
        ...
        this.requestMubkang.getComponent(Sprite).spriteFrame = GDDSCBASMR_DataManager.mukbangSfs.find((value, index, obj) => { if (value.name == `mold${GDDSCBASMR_DataManager.requestMubkang[0] + 1}-${GDDSCBASMR_DataManager.jellyName[GDDSCBASMR_DataManager.requestMubkang[1]]}_1`) return value; });
        this.requestMubkang.children[0].getComponent(Sprite).spriteFrame = GDDSCBASMR_DataManager.requestMubkang[3] == 0 ? null : GDDSCBASMR_DataManager.gameTop2.find((value, index, obj) => { if (value.name == `blink${GDDSCBASMR_DataManager.requestMubkang[3]}`) return value; });
        this.requestMubkang.children[1].getComponent(Sprite).spriteFrame = GDDSCBASMR_DataManager.top1.find((value, index, obj) => { if (value.name == GDDSCBASMR_DataManager.top1Name[GDDSCBASMR_DataManager.requestMubkang[2]]) return value; });
    }
    ...
    _InitStep() {
        ...
        switch (this.step) {
            ...
            case 2:
                this.isAni = true;
                GDDSCBASMR_AudioManager.Instance._PlaySound(50);
                data = GDDSCBASMR_DataManager.curMubkang;
                spine = this.mubkang.children[1].getComponent(sp.Skeleton);
                spine.setSkin(`color/${GDDSCBASMR_DataManager.jellyName[data[1]]}`);
                spine.animation = `mold${data[0] + 1}`;
                spine = this.mubkang.children[2].getComponent(sp.Skeleton);
                spine.node.active = true;
                spine.setSkin(`mold${data[0] + 1}/mold${data[0] + 1}-${GDDSCBASMR_DataManager.jellyName[data[1]]}`);
                spine.animation = `mold${data[0] + 1}`;
                time = spine.findAnimation(`mold${data[0] + 1}`).duration;
                this.scheduleOnce(() => {
            ...
            case 3:
                this.isAni = true;
                this.fan.getComponent(Animation).stop();
                particles = this.fan.children[0].getComponentsInChildren(ParticleSystem2D);
                for (let i = 0; i < particles.length; i++) {
                    const element = particles[i];
                    element.stopSystem();
                }
            ...
            case 4:
                ...
                num = 0;
                items = GDDSCBASMR_DataManager.unlockItemLevels[2];
                for (let j = 0; j < items.length; j++) {
                    let item = this.content.children[num];
                    if (!item) item = PoolManager.GetNodeByPrefab(this.itemPrefab, this.content);
                    item.getComponent(GDDSCBASMR_Item)._Init(2, j);
                    num++;
                }
                for (let i = items.length; i < this.content.children.length; i++) {
                    const element = this.content.children[i];
            ...
            case 5:
                ...
                tween(this.content.parent.parent.getComponent(Widget))
                    ...
                    .call(() => {
                        let num = 0;
                        items = GDDSCBASMR_DataManager.unlockItemLevels[3];
                        for (let j = 0; j < items.length; j++) {
                            let item = this.content.children[num];
                            if (!item) item = PoolManager.GetNodeByPrefab(this.itemPrefab, this.content);
                            item.getComponent(GDDSCBASMR_Item)._Init(3, j);
                            num++;
                        }
                        for (let i = items.length; i < this.content.children.length; i++) {
                            const element = this.content.children[i];
                ...
                break;
            case 6:
                GDDSCBASMR_MainPanel.Instance.completePanel.active = true;
                particles = this.top2.children[GDDSCBASMR_DataManager.curMubkang[3] - 1]?.getComponentsInChildren(ParticleSystem2D);
    ...
    NextStep() {
        if (this.step == 5 && GDDSCBASMR_DataManager.curMubkang[3] != 0) {
            this.next.active = false;
            this.isAni = true;
            tween(this.content.parent.parent.getComponent(Widget))
                .by(0.5, { bottom: -500 }, { easing: EasingType.backIn })
    ...
    Click() {
        ...
        if (this.clickTime > 2) {
            ...
            let data = GDDSCBASMR_DataManager.curMubkang;
            this.finishMubkang.setPosition(v3(0, 300));
            this.finishMubkang.getComponent(Sprite).spriteFrame = GDDSCBASMR_DataManager.mukbangSfs.find((value, index, obj) => { if (value.name == `mold${data[0] + 1}-${GDDSCBASMR_DataManager.jellyName[data[1]]}_1`) return value; });
            ...
            tween(this.finishMubkang)
                ...
                .call(() => { GDDSCBASMR_AudioManager.Instance._PlaySound(51); })
