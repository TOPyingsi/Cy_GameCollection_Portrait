...
@ccclass('QLED_ClearMask')
export default class QLED_ClearMask extends Mask {
    ...
    start() {
        ...
        director.getScene().on("清理耳朵_开始清理", this.TouchMove, this);
        director.getScene().on("清理耳朵_清理结束", this.touchEnd, this);
    ...
    touchMoveEvent(pos: Vec3, index: number, offsetX: number = 0, offsetY: number = 0) {
        ...
        switch (this.maskIndex) {
            case -2:
            case 1:
                QLED_GameMgr.instance.Wash();
                break;
            case -1:
            case 0:
                QLED_GameMgr.instance.Clear();
                break;
            default:
                break;
        }
    ...
    public touchEnd() {
        ...
        if (!this.isFinish) {
            ...
            if (this.maskIndex === 1) {
                ...
                QLED_GameMgr.instance.plasterNum++;

                if (QLED_GameMgr.instance.plasterNum === 9) {
                    QLED_GameMgr.instance.nextStep();
                    return;
                }
            ...
            if (this.maskIndex === 0) {
                if (!QLED_GameMgr.instance.dingPick) {
                    return;
                }
            }

            QLED_GameMgr.instance.nextClear(this.maskIndex + 2);
