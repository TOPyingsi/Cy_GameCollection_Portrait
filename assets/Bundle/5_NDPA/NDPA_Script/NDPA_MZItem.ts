...
@ccclass('NDPA_MZItem')
export class NDPA_MZItem extends Component {

    @property({ type: Enum(NDPA_NUMBER) })
    Number: NDPA_NUMBER = NDPA_NUMBER.NUMBER1;
    ...
    show() {
        ...
        if (NDPA_PrefsManager.Instance.userData.HaveMZ.find(e => e == this.Number)) {
            this.IsHave = true;
        ...
        if (NDPA_PrefsManager.Instance.userData.UseMZ == this.Number) {
            this.IsUsing = true;
    ...
    use() {
        ...
        if (this.IsUsing) {
            NDPA_PrefsManager.Instance.userData.UseMZ = this.Number;
            NDPA_PrefsManager.Instance.saveData();
        }
        NDPA_EventManager.Scene.emit(NDPA_MyEvent.NDPA_SHOWMZITEM);
    }
    ...
    buy() {
        ...
        if (NDPA_PrefsManager.Instance.userData.Gold >= this.Price) {
            ...
            NDPA_PrefsManager.Instance.userData.HaveMZ.push(this.Number);
            NDPA_PrefsManager.Instance.saveData();
    ...
    protected onEnable(): void {
        NDPA_EventManager.ON(NDPA_MyEvent.NDPA_SHOWMZITEM, this.show, this);
    }

    protected onDisable(): void {
        NDPA_EventManager.OFF(NDPA_MyEvent.NDPA_SHOWMZITEM, this.show, this);
    }
