...
@ccclass('SHJCB_ParttimePanel')
export class SHJCB_ParttimePanel extends SHJCB_UIBase {
    ...
    protected _InitData(): void {
        SHJCB_AudioManager.Instance._PlayPTMusic();
    ...
    StartGame() {
        ...
        this.scheduleOnce(() => {
            SHJCB_AudioManager.Instance._PlaySound(23);
    ...
    _GuestReady() {
        ...
        this.guest.getComponent(Sprite).spriteFrame = SHJCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_normal") return value; });
    ...
    _Win() {
        SHJCB_AudioManager.Instance._PlaySound(24);
        this.isOrder = false;
        this.tf[0].active = true;
        this.guest.getComponent(Sprite).spriteFrame = SHJCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_happy") return value; });
        this.GuestNum++;
    ...
    _Fail() {
        SHJCB_AudioManager.Instance._PlaySound(25);
        this.isOrder = false;
        this.tf[1].active = true;
        this.guest.getComponent(Sprite).spriteFrame = SHJCB_DataManager.guestSfs.find((value, index, obj) => { if (value.name == this.guestName + "_sad") return value; });
        this._GuestLeave();
    }
    ...
    VideoCoin() {
        ...
        Banner.Instance.ShowVideoAd(() => {
            ...
            SHJCB_DataManager.coinSpawn = coinSpawn.getWorldPosition();
    ...
    EndCoin() {
        SHJCB_AudioManager.Instance._PlaySound(1);
        SHJCB_AudioManager.Instance._PlayMusic();
        SHJCB_DataManager.coinSpawn = this.coinNode[1].getWorldPosition();
        let coin = SHJCB_DataManager.Instance.getNumberData("Coin");
        coin += 800;
        SHJCB_DataManager.Instance.setNumberData("Coin", coin);
        // let ani = this.finishPanel.getComponent(Animation);
