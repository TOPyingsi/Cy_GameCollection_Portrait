...
import { SHJCB_SupermarketItem } from './SHJCB_SupermarketItem';
...
@ccclass('SHJCB_SupermarketLivePanel')
export class SHJCB_SupermarketLivePanel extends SHJCB_UIBase {

    private static instance: SHJCB_SupermarketLivePanel;
    public static get Instance(): SHJCB_SupermarketLivePanel {
    ...
    eatNum = 0;
    canEat = false;
    eatTarget: SHJCB_SupermarketItem;

    protected onLoad(): void {
        super.onLoad();
        SHJCB_SupermarketLivePanel.instance = this;
    }

    protected _InitData(): void {
        this._UpdateSkin();
        let pickItems = SHJCB_SupermarketPanel.Instance.pickItems;
        for (let i = 0; i < this.items.children.length; i++) {
            ...
            item.getComponent(SHJCB_SupermarketItem).id = i;
        }
        this.eatNum = 0;
    ...
    TakePhoto() {
        SHJCB_AudioManager.Instance._PlaySound(6);
        this.shining.play();
    ...
    _Eat(item: SHJCB_SupermarketItem) {
        this.unschedule(this._Hungry);
        this.unschedule(this._Idle);
        this.canEat = false;
        this.playerState = PlayerState.Eat;
        this.player.loop = false;
        SHJCB_AudioManager.Instance._PlaySound(8 + item.sound);
        let str = "";
        switch (item.eatType) {
            case 0:
                str = "chidongxi";
        ...
        this.scheduleOnce(() => {
            ...
            if (this.eatTarget || this.headSocket.getComponent(UITransform).getBoundingBoxToWorld().intersects(item.getComponent(UITransform).getBoundingBoxToWorld())) this._Gape();
                ...
                SHJCB_AudioManager.Instance._PlaySound(16 + item.expressType);
                this.playerState = PlayerState.Express;
    ...
    EatEnd(item: SHJCB_SupermarketItem) {
        item._CancelTouch();
        SHJCB_DataManager.coinSpawn = item.node.getWorldPosition();
        let coins = SHJCB_DataManager.Instance.getNumberData("Coin");
        coins += item.price == 0 ? 200 : item.price;
        SHJCB_DataManager.Instance.setNumberData("Coin", coins);
        this.eatNum++;
        if (this.eatNum >= SHJCB_SupermarketPanel.Instance.pickItems.length) this.scheduleOnce(() => {
            SHJCB_MainPanel.Instance.supermarketEndPanel.active = true;
