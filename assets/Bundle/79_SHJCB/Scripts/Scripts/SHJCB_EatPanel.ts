...
import { PlayerState } from './SHJCB_SupermarketLivePanel';
import { SHJCB_AudioManager } from './SHJCB_AudioManager';
import { eventCenter } from './SHJCB_EventCenter';
import { ProjectEventManager, ProjectEvent } from 'db://assets/Scripts/Framework/Managers/ProjectEventManager';
const { ccclass, property } = _decorator;

@ccclass('SHJCB_EatPanel')
export class SHJCB_EatPanel extends SHJCB_UIBase {

    private static instance: SHJCB_EatPanel;
    public static get Instance(): SHJCB_EatPanel {
    ...
    _InitMubkang() {
        ...
        for (let i = 0; i < this.mubkangs.children.length; i++) {
            const element = this.mubkangs.children[i].children[1];
            if (SHJCB_DataManager.mukbangSfs) element.getComponent(Sprite).spriteFrame = SHJCB_DataManager.mukbangSfs.find((value, index, obj) => { if (value.name == `mold${SHJCB_DataManager.curMubkang[0] + 1}-${SHJCB_DataManager.jellyName[SHJCB_DataManager.curMubkang[1]]}_1`) return value; });
            else eventCenter.once("mukbangSfs", () => { element.getComponent(Sprite).spriteFrame = SHJCB_DataManager.mukbangSfs.find((value, index, obj) => { if (value.name == `mold${SHJCB_DataManager.curMubkang[0] + 1}-${SHJCB_DataManager.jellyName[SHJCB_DataManager.curMubkang[1]]}_1`) return value; }); }, this);
            element.children[0].getComponent(Sprite).spriteFrame = element.getComponent(Sprite).spriteFrame;
            element.children[0].children[0].getComponent(Sprite).spriteFrame = SHJCB_DataManager.curMubkang[3] == 0 ? null : SHJCB_DataManager.gameTop2.find((value, index, obj) => { if (value.name == `blink${SHJCB_DataManager.curMubkang[3]}`) return value; });
    ...
    _InitRequest() {
        ...
        let right = true;
        for (let i = 0; i < SHJCB_DataManager.curMubkang.length; i++) {
            if (SHJCB_DataManager.curMubkang[i] != SHJCB_DataManager.requestMubkang[i]) {
                right = false;
                break;
            }
        }
        ...
        tween(this.request)
            ...
            .call(() => {
                if (right) {
                    SHJCB_DataManager.coinSpawn = this.mubkang.getWorldPosition();
    ...
    _Eat() {
        ...
        SHJCB_AudioManager.Instance._PlaySound(12);
        let str = "chidongxi";
        ...
        if (num < 4) {
            this.eatTarget.getComponent(Sprite).spriteFrame = SHJCB_DataManager.mukbangSfs.find((value, index, obj) => { if (value.name == `mold${SHJCB_DataManager.curMubkang[0] + 1}-${SHJCB_DataManager.jellyName[SHJCB_DataManager.curMubkang[1]]}_${num}`) return value; });
            ...
            if (this.eatNum >= 3) this.scheduleOnce(() => {
                SHJCB_MainPanel.Instance.endPanel.active = true;
        ...
        this.scheduleOnce(() => {
            if (this.eatTarget && this.face.getComponent(UITransform).getBoundingBoxToWorld().intersects(this.eatTarget.getComponent(UITransform).getBoundingBoxToWorld())) this._Gape();
                ...
                SHJCB_AudioManager.Instance._PlaySound(16);
