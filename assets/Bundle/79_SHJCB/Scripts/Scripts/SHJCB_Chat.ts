...
@ccclass('SHJCB_Chat')
export class SHJCB_Chat extends Component {
    ...
    protected onEnable(): void {
        SHJCB_AudioManager.Instance._PlaySound(45);
        this.node.setPosition(Vec3.ZERO);
        this.node.children[1].setScale(Vec3.ZERO);
        this.uiOpacity.opacity = 255;
        if (this.sprite) {
            if (SHJCB_DataManager.chatSfs) this.sprite.spriteFrame = SHJCB_DataManager.chatSfs[randomRangeInt(0, SHJCB_DataManager.chatSfs.length)];
            else eventCenter.once("chatSfs", () => { this.sprite.spriteFrame = SHJCB_DataManager.chatSfs[randomRangeInt(0, SHJCB_DataManager.chatSfs.length)]; }, this);
