import { _decorator, Component, find, geometry, Material, math, PhysicsSystem, SkeletalAnimation, SkinnedMeshRenderer, tween, Vec3, Node } from 'cc';
...
@ccclass('XCJZ_PlayerController')
export class XCJZ_PlayerController extends Component {
    ...
    protected onLoad(): void {
        this.PlayerSkeleton = this.getComponent(SkeletalAnimation);
        this.BodyMesh = find("Body", this.node).getComponent(SkinnedMeshRenderer);
    }
    ...
    private raycastClosest() {
        ...
        if (block) {
            this.StepOn(block);
        } else if (XCJZ_GameManager.Instance.IsUnbeatable) {
            ...
            if (block) {
                ...
                this.totalTime = block.getComponent(XCJZ_BlockController).JumpTime;
    ...
    Win() {
        ...
        const starCount: number = Math.floor(math.clamp(3 - this._resurgenceCount * 0.5, 1, 3));
    ...
    StepOn(block: XCJZ_BlockController) {
        ...
        } else if (!XCJZ_GameManager.Instance.IsUnbeatable && this.CurColor != block.Block) {
            this.Fail();
            return;
        } else {
            this.PlayAni(XCJZ_ANI.Jump_Normal);
