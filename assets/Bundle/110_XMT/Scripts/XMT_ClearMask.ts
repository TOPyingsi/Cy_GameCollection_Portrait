import { _decorator, Component, director, Mask, Node, rect, Rect, Sprite, UITransform, v3, Vec2, Vec3 } from 'cc';
import { XMT_GameMgr } from './XMT_GameMgr';
const { ccclass, property } = _decorator;

@ccclass('XMT_ClearMask')
export class XMT_ClearMask extends Mask {
    @property()
    ...
    start() {
        ...
        director.getScene().on("修马蹄_开始修", this.TouchMove, this);
        director.getScene().on("修马蹄_结束修", this.touchEnd, this);
    ...
    touchMoveEvent(pos: Vec3, index: number, offsetX: number = 0, offsetY: number = 0) {
        ...
        if (this.inverted) {
            ...
            if (this.ClearRate * 100 <= this.drawMax) {
                ...
                XMT_GameMgr.instance.progressBar.progress = 1;
            ...
            if (this.JinDuTiao) {
                ...
                XMT_GameMgr.instance.progressBar.progress = this.drawMax / (this.ClearRate * 100);
            }
            ...
            if (this.JinDuTiao) {
                ...
                XMT_GameMgr.instance.progressBar.progress = (this.ClearRate * 100) / this.clearMax;
    ...
    public TouchStart(event, index: number, offsetX: number = 0, offsetY: number = 0) {
        ...
        // this.scheduleOnce(() => {
        //     // if () {
        //     director.getScene().emit("修马蹄_结束");
        //     director.getScene().emit("修马蹄_强制归位");
        //     // }
        // }, 2);
