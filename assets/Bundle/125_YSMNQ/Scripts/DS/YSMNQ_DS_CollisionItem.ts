...
import { YSMNQ_DS_ItemDefibrillator } from './YSMNQ_DS_ItemDefibrillator';
import { YSMNQ_DS_ItemQuilt } from './YSMNQ_DS_ItemQuilt';
const { ccclass, property } = _decorator;

@ccclass('YSMNQ_DS_CollisionItem')
export class YSMNQ_DS_CollisionItem extends YSMNQ_ItemBase {
        ...
        switch(this._itemId){
            ...
            case 1:
                this._secondPanel.getComponent(YSMNQ_ItemSelectedPanel).show((idx,spriteFrame)=>{
                    this._operationAnimsNode.getChildByName("operationAnims_"+targetId).getChildByName("chuizi").getComponent(Sprite).spriteFrame = spriteFrame;
                    callback&&callback();
                })
                break;
            case 3:
    ...
    showOperationAnim(targetId: number): void {
        ...
        switch(this._itemId){
            ...
            case 9:
                ...
                duration = animParentNode.children[0].getComponent(Animation).clips[0].duration*animParentNode.children[0].getComponent(Animation).clips[0].speed;
                this.scheduleOnce(()=>{
                    let itemQuilt= animParentNode.children[0].getComponent(YSMNQ_DS_ItemQuilt);
                    itemQuilt.init(()=>{
                        // animParentNode.children.forEach((animNode) => {
                        //     let animCom = animNode.getComponent(Animation);
                        //     if(animCom){
    ...
    showOperationTip(): void {
        switch(this._itemId){
            case 0:
                YSMNQ_DS_Manager.Instance.showTip("戴上氧气面罩");
                break;
            case 1:
                YSMNQ_DS_Manager.Instance.showTip("用锤子敲碎冰块");
                break;
            case 3:
                YSMNQ_DS_Manager.Instance.showTip("冰敷淤伤");
                break;
            case 4:
                YSMNQ_DS_Manager.Instance.showTip("用镊子拔出碎玻璃");
                break;
            case 5:
                YSMNQ_DS_Manager.Instance.showTip("用体温计量体温");
                break;
            case 6:
                YSMNQ_DS_Manager.Instance.showTip("用听诊器监测病人心率");
                break;
            case 7:
                YSMNQ_DS_Manager.Instance.showTip("给病人打针");
                break;
            case 8:
               YSMNQ_DS_Manager.Instance.showTip("用除颤器抢救病人");
                break;
            case 9:
                YSMNQ_DS_Manager.Instance.showTip("给病人盖上被子");
                break;
            default:
                break;
        }
