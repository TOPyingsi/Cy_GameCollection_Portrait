...
@ccclass('YSMNQ_HXZ_SecondPanel_0')
export class YSMNQ_HXZ_SecondPanel_0 extends Component {
    static Instance: YSMNQ_HXZ_SecondPanel_0 = null;
    ...
    itemGroupIds: {itemId:number,status:boolean}[][] = [
        ...
        [{itemId:5,status:false},{itemId:6,status:false}],
        [{itemId:7,status:false},{itemId:8,status:false}],
    ...
    onLoad(){
        YSMNQ_HXZ_SecondPanel_0.Instance = this;
        this.node.getComponent(Widget).updateAlignment();

        this.itemLayer.getComponent(Widget).updateAlignment();
        this.itemLayer.children.forEach((item, index) => {
            let count = item.children.length;
            item.getComponent(UITransform).width =  this.itemLayer.getComponent(UITransform).width/2*count;
