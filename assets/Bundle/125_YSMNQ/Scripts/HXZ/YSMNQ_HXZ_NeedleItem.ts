...
import { YSMNQ_HXZ_Manager } from './YSMNQ_HXZ_Manager';
import { YSMNQ_HXZ_PeopleState } from './YSMNQ_HXZ_PeopleState';
...
@ccclass('YSMNQ_HXZ_NeedleItem')
export class YSMNQ_HXZ_NeedleItem extends YSMNQ_ItemBase {
    ...
    handleSecondPanel(targetId:number,callback:()=>void): void {
        switch(this._itemId){
            case 0:
                this._secondPanel.getComponent(YSMNQ_HXZ_SecondPanel_0).init(()=>{
                    callback&&callback();
                })
                break;
            default:
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HXZ_Manager.Instance.showPeopleReact(YSMNQ_HXZ_PeopleState.迷糊);
                break;
            default:
                break;
        }
    ...
    showOperationSpTip(): void {
        YSMNQ_HXZ_Manager.Instance.showOperationSpTip(this._itemId);
    }

    hideOperationSpTip(): void {
        YSMNQ_HXZ_Manager.Instance.hideOperationSpTip();
    }

    showNormalTip(): void {
        YSMNQ_HXZ_Manager.Instance.showTip("治疗病人");
    }
    ...
    playAnim(targetId:number){
        let animName = "anim_"+this._count;
        let animNode = this._operationAnimsNode.children[0];
        animNode.active = true;
        let animCom = animNode.getComponent(Animation);
        if(animCom){
            animCom.play(animName)
        }
        let duration = animCom.getState(animName).duration*animCom.getState(animName).speed;
        this.scheduleOnce(()=>{
                let animCom = animNode.getComponent(Animation);
                if(animCom){
                    animCom.stop()
                ...
                this._operationAnimsNode.getChildByName("线_"+targetId).active = true
