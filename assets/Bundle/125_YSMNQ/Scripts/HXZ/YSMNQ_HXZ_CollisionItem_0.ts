...
@ccclass('YSMNQ_HXZ_CollisionItem_0')
export class YSMNQ_HXZ_CollisionItem_0 extends YSMNQ_ItemBase {
    ...
    showOperationTip(): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HXZ_Manager.Instance.showTip("给手术区盖上手术布");
                break;
            default:
                break;
        }
    ...
    passTarget(targetId: number): void {
        ...
        if(!isHave){
            ...
            YSMNQ_HXZ_Manager.Instance.passItem(this._itemId);
            this.showNormalTip();
            this.hideOperationSpTip();
        }
        else{
