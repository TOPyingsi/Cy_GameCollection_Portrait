...
@ccclass('YSMNQ_AudioManager')
export class YSMNQ_AudioManager extends Component {
    private static instance: YSMNQ_AudioManager;
    ...
    // 单例模式
    public static getInstance(): YSMNQ_AudioManager {
        if (!YSMNQ_AudioManager.instance) {
            // 查找场景中是否已存在GameManager节点
            const existingNode = director.getScene().getChildByName('GameManager');
            if (existingNode) {
                YSMNQ_AudioManager.instance = existingNode.getComponent(YSMNQ_AudioManager);
            } else {
                // 如果不存在，则创建新节点并添加组件
                const gameManagerNode = new Node('GameManager');
                director.getScene().addChild(gameManagerNode);
                YSMNQ_AudioManager.instance = gameManagerNode.addComponent(YSMNQ_AudioManager);
            }
        }
        return YSMNQ_AudioManager.instance;
    }

    protected onLoad(): void {
        YSMNQ_AudioManager.instance = this;
