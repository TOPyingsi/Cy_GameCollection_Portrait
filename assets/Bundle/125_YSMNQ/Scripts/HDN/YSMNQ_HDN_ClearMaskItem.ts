import { _decorator, Component, Graphics, Mask, Node, Rect, rect, UITransform, v3, Vec2, Vec3, EventTouch, Animation, tween, UIOpacity } from 'cc';
 ...
 */
@ccclass('YSMNQ_HDN_ClearMaskItem')
export default class YSMNQ_HDN_ClearMaskItem extends YSMNQ_ItemBase {
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HDN_SecondPanel_0.Instance.showPeopleReact(YSMNQ_HDN_PeopleState.迷糊);
                break;
            default:
                break;
        }
    ...
    private handleTouch(touchWorldPos: Vec3) {
        ...
        this._maskItemStates.forEach((itemState, index) => {
            ...
            this.clearMaskItem(itemState, localPos);
     ...
     */
    private clearMaskItem(itemState:ClearMaskItemState, localPos: Vec3) {
