...
@ccclass('YSMNQ_HDN_LongPressItem')
export class YSMNQ_HDN_LongPressItem extends YSMNQ_ItemBase {
    @property(Boolean)
    ...
    private reduceSpeed:number = 180;
        private reduceSpeed2:number = 0.7;
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 2:
                YSMNQ_HDN_SecondPanel_0.Instance.showPeopleReact(YSMNQ_HDN_PeopleState.咬紧牙关);
                break;
            default:
                break;
        }
    ...
    update(dt){
        if( this._isAllClear || !this._isCanMove )return;
        for (let i = 0; i < this._pressAreas.length; i++) {
             const pressArea = this._pressAreas[i];
            ...
            if (this._isPressing.get(id)) {
                if(!this.isScale){
                    ...
                    const spNode = pressArea.getChildByName("sp")
                    if (spNode && spNode.scale.x > 0) {
                        let reduce = this.reduceSpeed2 * dt;
                        spNode.setScale(v3(spNode.scale.x-reduce,spNode.scale.y-reduce,1));
                        if (spNode.scale.x <= 0) {
                            spNode.setScale(v3(0,0,1));
                            this.checkAllClear(id);
                        }
                    }
                }
    ...
    private _isAllClear: boolean = false;
     ...
     // 完成目标点
       passTarget(targetId: number): void {
           ...
           if(!isHave){
               ...
               EventManager.Scene.emit(YSMNQ_GameEvent.HDN_Item_LongPress_Completed);
               this.showNormalTip();
               this.hideOperationSpTip();
           }
           else{
