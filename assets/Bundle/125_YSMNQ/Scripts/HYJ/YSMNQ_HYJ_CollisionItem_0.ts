...
import { YSMNQ_HYJ_Manager } from './YSMNQ_HYJ_Manager';
...
export class YSMNQ_HYJ_CollisionItem_0 extends YSMNQ_ItemBase {
    ...
    handleSecondPanel(targetId:number,callback:()=>void): void {
        switch(this._itemId){
            case 0:
                this._secondPanel.getComponent(YSMNQ_HYJ_SecondPanel_0).init(()=>{
                    callback&&callback();
                })
                break;
            default:
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HYJ_Manager.Instance.showPeopleReact(YSMNQ_HYJ_PeopleState.迷糊);
                break;
            default:
                break;
        }
    ...
    showOperationSpTip(): void {
        YSMNQ_HYJ_Manager.Instance.showOperationSpTip(this._itemId);
    }

    hideOperationSpTip(): void {
        YSMNQ_HYJ_Manager.Instance.hideOperationSpTip();
    }

    showNormalTip(): void {
        YSMNQ_HYJ_Manager.Instance.showTip("治疗病人");
    }

    showOperationTip(): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HYJ_Manager.Instance.showTip("给手术区盖上手术布");
                break;
            default:
                break;
        }
    ...
    passTarget(targetId: number): void {
        ...
        if(!isHave){
            ...
            YSMNQ_HYJ_Manager.Instance.passItem(this._itemId);
            this.showNormalTip();
            this.hideOperationSpTip();
        }
        else{
