...
import { YSMNQ_HYJ_PeopleState } from './YSMNQ_HYJ_PeopleState';
...
@ccclass('YSMNQ_HYJ_LongPressItem')
export class YSMNQ_HYJ_LongPressItem extends YSMNQ_ItemBase {
    @property(Boolean)
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 2:
                YSMNQ_HYJ_SecondPanel_0.Instance.showPeopleReact(YSMNQ_HYJ_PeopleState.咬紧牙关);
                break;
            default:
                break;
        }
     ...
     // 完成目标点
       passTarget(targetId: number): void {
           ...
           if(!isHave){
               ...
               EventManager.Scene.emit(YSMNQ_GameEvent.HYJ_Item_LongPress_Completed);
               this.showNormalTip();
               this.hideOperationSpTip();
           }
           else{
