...
import { YSMNQ_HYC_Manager } from './YSMNQ_HYC_Manager';
...
@ccclass('YSMNQ_HYC_CollisionItem_0')
export class YSMNQ_HYC_CollisionItem_0 extends YSMNQ_ItemBase {
    ...
    handleSecondPanel(targetId:number,callback:()=>void): void {
        switch(this._itemId){
            case 0:
                this._secondPanel.getComponent(YSMNQ_HYC_SecondPanel_0).init(()=>{
                    callback&&callback();
                })
                break;
            default:
    ...
    showPeopleReact(targetId:number): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HYC_Manager.Instance.showPeopleReact(YSMNQ_HYC_PeopleState.迷糊);
                break;
            default:
                break;
        }
    ...
    showOperationSpTip(): void {
        YSMNQ_HYC_Manager.Instance.showOperationSpTip(this._itemId);
    }

    hideOperationSpTip(): void {
        YSMNQ_HYC_Manager.Instance.hideOperationSpTip();
    }

    showNormalTip(): void {
        YSMNQ_HYC_Manager.Instance.showTip("治疗病人");
    }

    showOperationTip(): void {
        switch(this._itemId){
            case 0:
                YSMNQ_HYC_Manager.Instance.showTip("用检测仪放大口腔区域");
                break;
            default:
                break;
        }
    ...
    passTarget(targetId: number): void {
        ...
        if(!isHave){
            ...
            YSMNQ_HYC_Manager.Instance.passItem(this._itemId);
            this.showNormalTip();
            this.hideOperationSpTip();
        }
        else{
