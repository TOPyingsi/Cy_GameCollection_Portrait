...
@ccclass('YSMNQ_HYC_TouchItem')
export class YSMNQ_HYC_TouchItem extends YSMNQ_ItemBase {
         ...
         this._targets.children.forEach((target) => {
            if(!isInside && target.active){
                ...
                if(isInside){
                    ...
                    this.showHole(targetId);
                    this.cancelTouch();
                    let allShow = true;
                    this._operationAnimsNode.children.forEach((target) => {
                        if(!target.active){
                            allShow = false;
                        }
                    })
    ...
    showHole(targetId:number): void {
        this._operationAnimsNode.getChildByName("hole_"+targetId).active = true;
    }
