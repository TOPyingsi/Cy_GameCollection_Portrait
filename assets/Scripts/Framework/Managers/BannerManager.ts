...
export enum BannerType {//广告类型
    ...
    解锁所有关卡 = "解锁所有关卡",//此类广告加在其他策略中，否则会重复执行
    开始游戏进入第一个游戏 = "开始游戏进入第一个游戏"//此类广告加在其他策略中，否则会重复执行
}
...
export class BannerManager extends Component {
    ...
    Init() {
        ...
        if (BannerManager.Strategy == Strategy.深圳腾逸) {
            if (window['htn'].getGGType == 1) {
                this.CurrentStrategy = this.StrategyMode11;//深圳腾逸策略GG1
            }
            if (window['htn'].getGGType == 2) {
                this.CurrentStrategy = this.StrategyMode12;//深圳腾逸策略GG2
            }
            if (window['htn'].getGGType == 3) {
                this.CurrentStrategy = this.StrategyMode13;//深圳腾逸策略GG3
            }
        }
    ...
    //#region 策略
    //规范: [广告类型, 屏蔽规则1, 屏蔽规则2.....],可以填入广告属性，ID，Top,Left
    ...
    public StrategyMode1 = {
        ...
        其他策略: [[BannerType.解锁所有关卡]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件（不要将这些事件填入其他策略中反复调用）
    }
    //策略2(王勇VR黑包)
    public StrategyMode2 = {
        首次主场景策略: [[BannerType.二次原], [MaskType.时间屏蔽]],
        游戏开始策略: [[MaskType.时间屏蔽], [BannerType.原生], [BannerType.Banner]],
        游戏结束策略: [[MaskType.时间屏蔽], [BannerType.原生], [BannerType.Banner]],
        弹出窗口策略: [[BannerType.原生]],
        页面转换策略: [],
        返回主页策略: [[BannerType.原生]],
        其他策略: [[BannerType.解锁所有关卡], [BannerType.三十秒自弹原生], [BannerType.三十五秒自弹添加桌面]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略3(深圳正常包)
    public StrategyMode3 = {
        ...
        页面转换策略: [],
        返回主页策略: [[BannerType.Banner]],
        其他策略: [[BannerType.解锁所有关卡]],//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略4(深圳VRGG1)
    public StrategyMode4 = {
        ...
        页面转换策略: [],
        返回主页策略: [],
        其他策略: [[BannerType.解锁所有关卡], [BannerType.五分钟自弹宝箱]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略5(深圳VRGG2)
    public StrategyMode5 = {
        ...
        其他策略: [[BannerType.解锁所有关卡], [BannerType.四十秒自弹原生], [BannerType.三十五秒自弹添加桌面]],//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略4(深圳ORGG1)
    ...
    public StrategyMode8 = {
        ...
        其他策略: [[BannerType.解锁所有关卡], [BannerType.三十秒自弹原生, MaskType.时间屏蔽], [BannerType.三分钟自弹视屏, MaskType.时间屏蔽]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略7(孙波策略)
    public StrategyMode9 = {
        ...
        返回主页策略: [],
        其他策略: [[BannerType.解锁所有关卡], [BannerType.开始游戏进入第一个游戏]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略8(万总华为策略)
    public StrategyMode10 = {
        ...
        页面转换策略: [[BannerType.原生]],
        返回主页策略: [],
        其他策略: [[BannerType.解锁所有关卡], [BannerType.三十秒自弹原生, MaskType.地区屏蔽], [BannerType.三十秒自弹Banner, MaskType.地区屏蔽]]//其他策略是指30s自弹，20s自弹，5分钟自弹宝箱等事件
    }
    //策略3(深圳腾逸GG1)
        ...
        for (let i = 0; i < data.length; i++) {
            if (this.GetBannerIsShow(data[i])) {
                ...
                if (data[i][0] == BannerType.四十秒自弹原生) {
                    Banner.Instance.StartPopupAd(20, BannerID == "" ? null : BannerID);
                }
                ...
                if (data[i][0] == BannerType.解锁所有关卡) {
                    DataManager.IsUnlockAll = true;
                }
                ...
                if (data[i][0] == BannerType.开始游戏进入第一个游戏) {
                    DataManager.EnterFirstGame = true;
                }
            }
        }
    }
